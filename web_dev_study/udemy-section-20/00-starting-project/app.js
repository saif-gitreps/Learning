...
app.get("/restaurants/:id", (request, response) => {
   ...
   // step 4 : Now according to the html logic of the subroute , we match using id or the selected card and send the entire object from the arrays of object [if youre confused just reread and understand it], {next-step- -> restaurant=detail}.
   //[note 'item' is the variable name that will be in the restauarent-details and we are passing an iterator from arrays of objects]
...
app.post("/recommend", (request, response) => {
   ...
   restaurant.id = uuid.v4(); // v4() method will give randomly generate unqiue id, in string. A new property in the newly submitted form in the form of 'id' will be added .
   const filePath = path.join(__dirname, "data", "restaurants.json");
   const fileData = fs.readFileSync(filePath);
   const storedRestData = JSON.parse(fileData);
